 ...
 * */
interface card_view {
    fun card_info_view(result: Boolean, info: CardInfoModel, type: Boolean, mes: String);
}

class ZJSBListener {
    //身份证识别
    fun cardRecognition_img(img: String, main: card_view, type: Boolean) {
        OkGo.post(app_url.url_scan_img + "CardRecognition")
                ...
                .execute(object : JsonCallback<LzyResponse<CardInfoModel>>() {
                    override fun onSuccess(s: LzyResponse<CardInfoModel>, call: Call, response: Response) {
                        if (s.Data != null) {
                            main.card_info_view(s.Success, s.Data!!, type, "证件识别成功")
                        } else if (s.Data == null) {
                            main.card_info_view(s.Success, CardInfoModel(), type, s.Message!!)
                        } else {
                            main.card_info_view(s.Success, s.Data!!, type, "")
                        }
                    }
                    ...
                    override fun onError(call: Call?, response: Response?, e: Exception?) {
                        ...
                        main.card_info_view(false, model!!, type, "网络错误")
                    }
                })
    }
}